...
const float Br = 0.0020; // Rayleigh coefficient
const float Bm = 0.0009; // Mie coefficient
...
void main()
{
  ...
  // Atmosphere Scattering: simplified (Mie + Rayleigh) scattering model
  float mu = dot(normalize(pos), normalize(fsun));
  vec3 extinction = mix(exp(-exp(-((pos.y + fsun.y * 4.0) * (exp(-pos.y * 16.0) + 0.1) / 80.0) / Br) * (exp(-pos.y * 16.0) + 0.1) * Kr / Br) * exp(-pos.y * exp(-pos.y * 8.0 ) * 4.0) * exp(-pos.y * 2.0) * 4.0, vec3(1.0 - exp(fsun.y)) * 0.2, -fsun.y * 0.2 + 0.5);
  color.rgb = 3.0 / (16.0 * 3.14) * (1.0 + mu * mu) * (Kr + Km * (1.0 - g * g) / (2.0 + g * g) / pow(1.0 + g * g - 2.0 * g * mu, 1.5)) / (Br + Bm) * extinction;
  ...
  for (int i = 0; i < 4; i++)
  {
    float density = smoothstep(1.0 - cumulus, 1.0, fbm((0.7 + float(i) * 0.01) * pos.xyz / pos.y + time * 0.15));
